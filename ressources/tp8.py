...
def greyscale(mat_img) :
    grey_img = []
    #on peu également faire une copie de l'image de base
    for i in range(len(mat_img)) : #ligne
        grey_img.append([])
        for j in range(len(mat_img[i])) : #colonne
            grey_img[i].append((0, 0, 0))
            R, G, B = mat_img[i][j]
            new_value = int((0.2125 * R) + (0.7154 * G) + (0.0721 * B))
